...
DASH_PATH = "frontend/src/components/Dashboard.jsx"

NEW_DASH = """import { useState, useEffect } from 'react'
...
export default function Dashboard({ onSelectCall }) {
            ...
            <tbody>
              {calls.map((call) => (
                <tr key={call.id} className="border-b border-black/5 hover:bg-black/5 transition-colors">
                  ...
                  <td className="p-4">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-background border border-black/10 text-textMain">
                  ...
                  <td className="p-4 text-right">
                    <button 
                      onClick={() => call.status === 'Concluído' && onSelectCall(call.id)}
